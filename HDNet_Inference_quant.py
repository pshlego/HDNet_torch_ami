...
from training.training_code.utils.Loss_functions_torch import calc_loss_normal2_1, calc_loss_normal_1, calc_loss_1, calc_loss_d_refined_mask_1
...
model_n.load_state_dict(torch.load(pre_ck_pnts_dir_NP_torch + 'model_1_11398.pt'))
...
model_d.load_state_dict(torch.load(pre_ck_pnts_dir_DR_torch + 'model_11398.pt'))
...
for i in range(num_sub):
    ...
    for f in range(len(test_files)):
        ...
        normal_pred_raw_1  = torch.Tensor(nmap_normalization(np.asarray(prediction1n)[0,...])).type(torch.float32).to(device)
        normal_pred_raw_torch_1  = torch.Tensor(nmap_normalization(np.asarray(prediction1n_torch.detach().numpy()))).type(torch.float32).to(device)
